...
- **`user_agent` is forwarded by the four default boards, and ignored by five upstream ones.** JobStreet, Hiredly, LinkedIn and Glassdoor send a caller's `user_agent` on their requests. Indeed stores it (`self.user_agent`) but deliberately keeps its iPhone-app UA on the API: a desktop browser UA got a 403 from that API (measured 2026-09-24), and `scrape_jobs` hands every board the same `user_agent`, so honouring it would let a UA meant for LinkedIn get Indeed blocked. ZipRecruiter, Google, Bayt, Naukri and BDJobs never forward it, and ZipRecruiter also never sets `self.ca_cert` (it passes the local param straight to `create_session`, so proxy CA certs still work). Those five are grandfathered as `xfail` in `USER_AGENT_NOT_FORWARDED` in the contract test — **a new board must not join that list**, and a board fixed must leave it, since the xfail skips the check entirely.
