...
- **JobStreet is the only board with structured MY salary.** It publishes a `salaryLabel` on most postings (measured 61.5% direct-data fill on a 52-job single-board run during this task, 66.67% on an earlier 24-job smoke test), used directly and marked `salary_source="direct_data"` — Indeed MY and LinkedIn both measure 0% direct-data salary fill, so JobStreet's numbers are what those boards are compared against. Note the multi-board baseline harness (`jobspy/baseline/runner.py`) passes one shared `location` string per search in the Indeed/LinkedIn convention (`"City, Malaysia"`); JobStreet's `where` param does not resolve that suffixed form the same way and can return far fewer rows than a bare place name (`"Kuala Lumpur"`) — a query-construction quirk of the fixed search set, not a scraper defect, but it means a multi-board baseline run understates JobStreet's contribution unless queried with a JobStreet-shaped location on its own.
