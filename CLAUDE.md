...
- **JobStreet is the only board with structured MY salary.** It publishes a `salaryLabel` on most postings, used directly and marked `salary_source="direct_data"` — Indeed MY and LinkedIn both measure 0% direct-data salary fill, so JobStreet's numbers are what those boards are compared against. No committed baseline breaks salary fill out per site yet (`jobspy/baseline/metrics.py` now computes `salary_fill_rate_by_site`, but no report has been regenerated against it) — the best current estimate, ≈50% for JobStreet, is derived in README.md's salary-parsing caveat by comparing two committed baseline runs with and without JobStreet, not read directly off a report. Don't quote a JobStreet-specific salary percentage here or in README.md without pointing at a committed report that actually breaks it out by site.
